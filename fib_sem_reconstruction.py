...
SCALING = [0.08e-6, 0.08e-6, 0.05e-6]
...
def get_aggregates_and_write_to_file(tomo, phase="voids", data_shape=(500, 500, 202), workdir="output/segmentation"):
    ...
    for i in range(n_ero_dil):
        pv_sieved_e_d = pv_sieved_e_d.image_dilate_erode(dilate_value=0, erode_value=1, kernel_size=(ks, ks, ks))
        
    for i in range(n_ero_dil + 1):
        pv_sieved_e_d = pv_sieved_e_d.image_dilate_erode(dilate_value=1, erode_value=0, kernel_size=(ks, ks, ks))
    ...
    cleaned = False
    # iters = 0
    # while not cleaned and iters < 10:
    #     print(iters, cleaned)
...
def create_volumes_from_stl(phase, workdir):
    ...
    #     print(i)
    gmsh.merge(os.path.join(workdir, "cam.stl"))
...
if __name__ == '__main__':
    ...
    # tomo3 = np.zeros((500, 500, 202), dtype=np.bool)
    ...
    #     # quit()
    #     tomo3[:, :, img_id - 1] = np.isclose(img_cam, 2)
    #     # tomo[np.isclose(img_voids, 1), img_id - 1] = 1#np.isclose(img_voids, 1)
    # tomo3[:10, :, :] = 1
    # # tomo[LX+1:, LY+1:, LZ+1:] = 0
    # tomo = np.zeros((LX+1, LY+1, LZ+1), dtype=bool)
    # tomo[:, :, :] = tomo3[:LX+1, :LY+1:, :LZ+1]
    # tomo[0, :, :] = 0
    # tomo[-1, :, :] = 0
    # tomo[:10, 0, :] = 0
    # tomo[:10, -1, :] = 0
    ...
    # gmsh.option.setNumber("Mesh.MeshSizeMin", 1e-7)
    ...
    tol = 1e-8
    lx = max(x_vals) + tol#- min(x_vals)
    ly = max(y_vals) + tol#- min(y_vals)
    lz = max(z_vals) + tol#- min(z_vals)
    sloop = create_box_surface_loop(Lx=lx, Ly=ly, Lz=lz, L_sep=L_sep, origin=(min(x_vals)-tol, min(y_vals), min(z_vals)))
    ...
    for surf in surfs:
        xmin, ymin, zmin, xmax, ymax, zmax = gmsh.model.get_bounding_box(*surf)
        if np.isclose(xmax, np.min(x_vals), atol=tol):
            right_surfs.append(surf[1])
        elif np.isclose(xmin, lx + args.L_sep, atol=tol):
    ...
    # gmsh.model.mesh.field.setNumber(2, "SizeMin", 0.1 * micron)
    # gmsh.model.mesh.field.setNumber(2, "SizeMax", 1.0 * micron)
    # gmsh.model.mesh.field.setNumber(2, "DistMin", 0.1 * micron)
    # gmsh.model.mesh.field.setNumber(2, "DistMax", 1.0 * micron)
