...
if __name__ == "__main__":
    ...
    for idx, piece in enumerate(pieces):
        ...
        meshio.write(f"mesh/s{grid_info}o{origin_str}_tetr{idx}.xdmf", tetra_mesh)
        logger.info(f"{idx}: %.4f %.4f" % (np.average(largest_piece[:, 0, :]), np.average(largest_piece[:, Ny - 1, :])))