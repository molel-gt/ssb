...
int main(int argc, char* argv[])
{
  ...
  loguru::set_thread_name(thread_name.c_str());
  {
    ...
    auto a = std::make_shared<fem::Form<T>>(fem::create_form<T>(
        *form_separator_mechanics_J_form, {V, V}, {{"u", u}}, {}, {}));
    auto L = std::make_shared<fem::Form<T>>(fem::create_form<T>(
        *form_separator_mechanics_F_form, {V}, {{"u", u}}, {}, {}));
    ...
    nls::petsc::NewtonSolver newton_solver(domain->comm());
    ...
    const auto cell_type
        = mesh::cell_type_to_basix_type(domain->topology()->cell_types()[0]);
    constexpr int k = 0;
    ...
    auto S = std::make_shared<fem::FunctionSpace<U>>(fem::create_functionspace(
        domain, S_element, std::vector<std::size_t>{3, 3}));

    auto sigma_expression = fem::create_expression<T, U>(
        *expression_separator_mechanics_sigma, {{"u", u}}, {});
