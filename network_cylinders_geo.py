...
if __name__ == '__main__':
    ...
    parser.add_argument("-n", "--name_of_study", help="name_of_study", nargs='?', const=1, default="network_cylinders")
        ...
        for x in lxs:
            for y in lys:
                ...
                if p_val <= 0.25:
                    ...
                    cyl = gmsh.model.occ.addCylinder(x, y, z_pos, 0, r/L_CELL, 0, r/L_CELL)
    ...
    for v in cylinders:
        print(v)
        e = gmsh.model.getBoundary(gmsh.model.getBoundary([v]), False)
        gmsh.model.occ.fillet([v[1]], [abs(i[1]) for i in e], [tol])
    # ov = gmsh.model.occ.fillet([v[1] for v in vols[1:]], [i[1] for i in join if i[0] == 1], [tol])
    gmsh.model.occ.synchronize()
    # print(ov)
    # vols = [v[1] for v in ov]
    # print(vols)
    vols = gmsh.model.getEntities(3)
    ov, ovv = gmsh.model.occ.fuse(vols[:1], vols[1:])
    gmsh.model.occ.synchronize()
    gmsh.model.occ.synchronize()
