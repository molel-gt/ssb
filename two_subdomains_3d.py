...
if __name__ == '__main__':
    ...
    objs = gmsh.model.occ.revolve(old_surfs, 1, 0, 0, 1, 0, 0, 2*np.pi, heights=[0.4], recombine=True)
    gmsh.model.occ.synchronize()
    vols = [tag for tag in objs if tag[0] == 3]
    surfs = [tag for tag in objs if tag[0] == 2 and tag[1] not in [v[1] for v in old_surfs]]
    for vol in vols:
        com = gmsh.model.occ.getCenterOfMass(*vol)
        if np.isclose(com[0], 0.25):
            gmsh.model.addPhysicalGroup(3, [vol[1]], markers.electrolyte, "electrolyte")
        elif np.isclose(com[0], 0.75):
            gmsh.model.addPhysicalGroup(3, [vol[1]], markers.positive_am, "positive_am")
        else:
            print(vol)
    left = []
    right = []
