...
def create_mesh(output_meshfile, markers, L=1, w_over_L=0.1, h_over_L=0.1, resolution=0.5e-3, refine=True, **kwargs):
    """
    """
    points_bottom = [
        (0, 0, 0),
...
if __name__ == '__main__':
    ...
    simulation_metadata_file = os.path.join(workdir, 'simulation.json')
    ...
    q_tilde_in =  comm.allreduce(fem.assemble_scalar(fem.form(inner(q_h, n) * ds(markers.left), entity_maps=entity_maps)), op=MPI.SUM) * norm_factor
    q_tilde_in =  comm.allreduce(fem.assemble_scalar(fem.form(inner(q_h, n) * ds(markers.right), entity_maps=entity_maps)), op=MPI.SUM) * norm_factor
    simulation_metadata = {
