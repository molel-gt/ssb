...
if __name__ == '__main__':
    ...
    if args.alpha > 0:
        F_0 += + args.alpha * kappa_elec * gamma * h_avg * inner(kappa_elec * grad(u_l) - kappa_pos_am * grad(u_r), grad(v_l)) * dInterface
        F_1 += - args.alpha * kappa_pos_am * gamma * h_avg * inner(kappa_elec * grad(u_l) - kappa_pos_am * grad(u_r), grad(v_r)) * dInterface
    ...
    while t < TIME:
        ...
        I_x_norm_sq = np.sqrt(comm.allreduce(fem.assemble_scalar(
                                    fem.form(inner(i_x, i_x) * L_ref ** 2 * dInterface, entity_maps=entity_maps)), op=MPI.SUM))

        I_interface_error_norm_sq = np.sqrt(comm.allreduce(fem.assemble_scalar(
    ...
    metadata = {
        ...
        "I_interface error norm (normalized)": np.sqrt(I_interface_error_norm_sq / I_x_norm_sq),
        "C-rate": args.C_rate,
        "u (avg) right [V]": u_avg_right,
        "u (stdev) right [v]": u_stdev_right,
        "i (avg) left [A/m2]": i_avg_left,
