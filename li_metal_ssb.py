...
if __name__ == '__main__':
    ...
    F_2 = (c - c0) * q * dx_r + dt * inner(ufl.grad(c), ufl.grad(q)) * dx_r
    F_2 += -dt * inner(kappa_total * phi_ref/(D * faraday_const * c_ref)/2 * (kappa_l * grad(u_l) + kappa_r * grad(u_r)), n_r) * q_r * dInterface
