...
if __name__ == '__main__':
    ...
    cdf_values = []
    for v in cd_space:
        lpvalue = fem.assemble_scalar(fem.form((np.asarray(ufl.inner(current_h, n)) <= v) * ds(markers.left_cc))) / area_left_cc
        rpvalue = fem.assemble_scalar(fem.form((np.asarray(ufl.inner(current_h, n)) <= v) * ds(markers.right_cc))) / area_right_cc
